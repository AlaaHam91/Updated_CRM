...
export default {
  ...
  mutations: {
    ...
    areaItems(state, data) {
      state.areaItems = data;
    },
  ...
  actions: {
    ...
    async loadItem({ state, dispatch }, data) {
      ...
      state.area = data.areas;
      state.shareWith = data.share_with[0];
      state.code = data.code;
      state.accountNo = data.account_number;
    ...
    reset({ state, dispatch }) {
      ...
      if (state.contractModule)
        dispatch(`${state.contractModule}/clearData`, null, { root: true });
    },
    touchDatatables({ state, commit }) {
      ...
      if (state.contractModule)
        commit(`${state.contractModule}/touchValidation`, null, { root: true });
    },
    loadTheInitial({ commit, dispatch }) {
      ...
      commit("productItems", [{ nameL: "pro1", name: "pro1", id: 1 }]);
      commit("areaItems", [{ nameL: "Area1", name: "Area1", id: 1 }]);
      userFieldsApi
        .getFor("Contact")
    ...
    loadFromModules({ state, rootGetters }) {
      ...
      if (state.contractModule)
        state.contractItems = rootGetters[`${state.contractModule}/items`];
    ...
    async initiateData({ state, dispatch }) {
      ...
      for (let j = 0; j < state.product.length; j++)
        data[`products[${j}]`] = await state.product[j];

      for (let j = 0; j < state.area.length; j++)
        data[`areas[${j}]`] = await state.area[j];
      ...
      for (let j = 0; j < state.contractItems.length; j++) {
        if (state.contractItems[j].status == "none") continue;

        data[`contracts[${i}][status]`] = state.contractItems[j].status;
        data[`contracts[${i}][number]`] = state.contractItems[j].number;
        data[`contracts[${i}][start_date]`] = state.contractItems[j].startDate;
        data[`contracts[${i}][end_date]`] = state.contractItems[j].endDate;
        data[`contracts[${i}][internal_visits]`] =
          state.contractItems[j].internalvisits;
        data[`contracts[${i}][external_visits]`] =
          state.contractItems[j].externalVisits;
        data[`contracts[${i}][support_hours]`] =
          state.contractItems[j].supportHours;
        data[`contracts[${i}][training_hours]`] =
          state.contractItems[j].trainingHours;
        i++;
      }
