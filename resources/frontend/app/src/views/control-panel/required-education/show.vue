<template>
  <card
    ...
    save-btn
    save-new-btn
    :delete-btn="form.id ? $can('Pg_req_education', 'Delete') : false"
  ...
  methods: {
    doSave() {
      return new promise((resolve, reject) => {
        if (this.form.id == 0 && !this.$canMsg("Pg_req_education", "Create"))
          reject();
        if (this.form.id > 0 && !this.$canMsg("Pg_req_education", "Update"))
          reject();
        this.$v.$touch();
        if (this.$v.$invalid) {
          this.$notify({
    ...
    save() {
      this.doSave()
        .then(() => {
          this.$router.push({ name: "required-education-index" });
        })
        .catch(() => {});
    },
    saveAndNew() {
