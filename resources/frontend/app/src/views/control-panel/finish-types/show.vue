<template>
  <card
    ...
    :new-btn="$can('Cp_other_finishType', 'Create')"
    save-btn
    save-new-btn
    :delete-btn="form.id ? $can('Cp_other_finishType', 'Delete') : false"
...
import api from "@/services/api/control-panel/finish-type.api.js";
  ...
  methods: {
    doSave() {
      return new promise((resolve, reject) => {
        if (this.form.id == 0 && !this.$canMsg("Cp_other_finishType", "Create"))
          reject();
        if (this.form.id > 0 && !this.$canMsg("Cp_other_finishType", "Update"))
          reject();
        this.$v.$touch();
        if (this.$v.$invalid) {
          this.$notify({
    ...
    save() {
      this.doSave()
        .then(() => {
          this.$router.push({ name: "finish-type-index" });
        })
        .catch(() => {});
    },
    saveAndNew() {
