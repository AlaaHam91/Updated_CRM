...
export default {
  name: "communication-method-faxes",
  ...
  methods: {
    ...
    async load() {
      ...
      setTimeout(() => {
        // wait for this.faxModule
        this.items.forEach((item, i) => {
          this.$store.commit(`${this.faxModule}/dedicatedOptions`, {
            index: i,
    ...
    async setPattern(id, val) {
      await this.$store.dispatch(`${this.faxModule}/editItem`, {
        id: id,
        pattern: val
      });
    },
  ...
  computed: {
    ...
    mobileLengthsCol() {
      if (!this.faxModule) return [];
      let items = this.$store.getters[`${this.faxModule}/items`];
      let lengthsItems = [];
  ...
  watch: {
    ...
    async countryCol(newVal, oldVal) {
      ...
      if (this.countryCol[index]) {
        ...
        await this.$store.commit(`${this.faxModule}/dedicatedOptions`, {
          index: index,
          optionsName: "administrativeDivisionOptions",
