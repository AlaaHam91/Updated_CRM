<template>
  <card
    ...
    :new-btn="$can('Pg_activities', 'Create')"
    save-btn
    save-new-btn
    :delete-btn="form.id ? $can('Pg_activities', 'Delete') : false"
    @first="first"
    @previous="previous"
    @next="next"
    @last="last"
...
import api from "@/services/api/control-panel/activity.api.js";
  ...
  methods: {
    doSave() {
      return new promise((resolve, reject) => {
        if (this.form.id == 0 && !this.$canMsg("Pg_activities", "Create"))
          reject();
        if (this.form.id > 0 && !this.$canMsg("Pg_activities", "Update"))
          reject();
        this.$v.$touch();
        if (this.$v.$invalid) {
          this.$notify({
