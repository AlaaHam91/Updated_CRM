<template>
  <card
    ...
    save-btn
    save-new-btn
    :delete-btn="form.id ? $can('Cp_jobType', 'Delete') : false"
  ...
  methods: {
    doSave() {
      return new promise((resolve, reject) => {
        if (this.form.id == 0 && !this.$canMsg("Cp_jobType", "Create"))
          reject();
        if (this.form.id > 0 && !this.$canMsg("Cp_jobType", "Update")) reject();
        this.$v.$touch();
        if (this.$v.$invalid) {
          this.$notify({
    ...
    save() {
      this.doSave()
        .then(() => {
          this.$router.push({ name: "job-type-index" });
        })
        .catch(() => {});
    },
    saveAndNew() {
