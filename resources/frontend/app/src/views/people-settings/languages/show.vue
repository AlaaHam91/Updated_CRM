<template>
  <card
    ...
    save-btn
    save-new-btn
    :delete-btn="form.id ? $can('Pg_languages', 'Delete') : false"
    ...
    <v-row>
      ...
      <v-col cols="12" md="6">
        <div
          ...
          v-text="$t('isFavorite')"
        ></div>
        <v-checkbox v-model="form.isFavorite"> </v-checkbox>
      </v-col>
    </v-row>
  </card>
</template>
  ...
  methods: {
    doSave() {
      return new promise((resolve, reject) => {
        if (this.form.id == 0 && !this.$canMsg("Pg_languages", "Create"))
          reject();
        if (this.form.id > 0 && !this.$canMsg("Pg_languages", "Update"))
          reject();
        this.$v.$touch();
        if (this.$v.$invalid) {
          this.$notify({
        ...
        let data = {
          ...
          is_favorite: this.form.isFavorite == true ? 1 : 0
        };
