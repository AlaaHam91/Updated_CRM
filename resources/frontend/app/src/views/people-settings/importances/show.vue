<template>
  <card
    ...
    @next="next"
    @last="last"
    :new-btn="$can('Pg_importance', 'Create')"
    save-btn
    save-new-btn
    :delete-btn="form.id ? $can('Pg_importance', 'Delete') : false"
...
import api from "@/services/api/people-settings/importance.api.js";
  ...
  methods: {
    doSave() {
      return new promise((resolve, reject) => {
        if (this.form.id == 0 && !this.$canMsg("Pg_importance", "Create"))
          reject();
        if (this.form.id > 0 && !this.$canMsg("Pg_importance", "Update"))
          reject();
        this.$v.$touch();
        if (this.$v.$invalid) {
          this.$notify({
    ...
    save() {
      this.doSave()
        .then(() => {
          this.$router.push({ name: "importance-index" });
        })
        .catch(() => {});
    },
    saveAndNew() {
